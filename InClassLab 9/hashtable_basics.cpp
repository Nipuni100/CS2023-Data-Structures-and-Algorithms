...
struct HashTable{
	...
	for (int i = 0; i < l; i++) 
        {
            convert = user_name[i];
            sum += convert; // Add the ASCII value of each character to the sum
    }
	...
	//add an if condition to complete the code here
        if(empty){
            ...
            cout<<"Collision occured! Action Required!!!"<<endl;
        }
	...
	//add an if condition to complete the code here
        if(empty){
            ...
            cout<<"Password of the user "+user_name+" : "+password[hash]<<endl;
        }
